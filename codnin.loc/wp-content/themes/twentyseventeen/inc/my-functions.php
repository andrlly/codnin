    ...
    class WCCPT_Product_Data_Store_CPT extends WC_Product_Data_Store_CPT
    {
        ...
        public function read(&$product)
        {
            ...
            if (!$product->get_id() || !($post_object = get_post($product->get_id())) || !in_array($post_object->post_type, array('films', 'product'))) { // change birds with your post type
                throw new Exception(__('Invalid product.', 'twentyseventeen'));
            }
            $id = $product->get_id();
...
function reigel_woocommerce_get_price($price,$post){
    ...
    if ($post_type === 'films') // change this to your post type
        $price = get_post_meta($post->get_id(), "price_film", true); // assuming your price meta key is price
    return $price;
}
...
/*************  ADD SKYPE FIELD TO REGISTER FORM  ********/
/*********************************************************/
add_action( 'woocommerce_register_form_start', 'wooc_extra_register_fields', 10, 3 );
function wooc_extra_register_fields() { ?>
    <p class="form-row form-row-wide">
        <label for="reg_billing_phone"><?php _e( 'Skype', 'twentyseventeen' ); ?></label>
        <input type="text" class="input-text" name="billing_phone" id="reg_billing_phone" value="<?php if ( ! empty( $_POST['billing_phone'] ) ) esc_attr_e( $_POST['billing_phone'] ); ?>" />
    </p>
    <p class="form-row form-row-first">
        <label for="reg_billing_first_name"><?php _e( 'First name', 'twentyseventeen' ); ?><span class="required">*</span></label>
        <input type="text" class="input-text" name="billing_first_name" id="reg_billing_first_name" value="<?php if ( ! empty( $_POST['billing_first_name'] ) ) esc_attr_e( $_POST['billing_first_name'] ); ?>" />
    </p>
    <p class="form-row form-row-last">
        <label for="reg_billing_last_name"><?php _e( 'Last name', 'twentyseventeen' ); ?><span class="required">*</span></label>
